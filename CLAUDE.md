...
### Session 12 — Security Hardening & Performance Audit ⏳ Pending — **NEXT**
...
│       │   ├── migrations/          # …_add_user_model, …_add_model_profile, …_add_content_management, …_add_payments, …_remove_ppv, …_add_payouts, …_add_payout_email, …_add_subscription_lifecycle, …_add_messaging, …_add_generation_jobs, …_nullable_content_storage_key, …_add_user_preferred_locale, …_add_admin_dashboard
...
- **A crash mid-generation leaves a stale `PENDING` `GenerationJob`** (Session 08) — the one-in-flight-per-subscriber constraint means a process crash between the credit debit and the provider response leaves that subscriber locked out (and the credits already spent) until the row is manually resolved. No sweeper reconciles this yet. **Still open after Session 11**; fold into the reconciliation job for Session 12.
- **No `Payout` reconciliation job** (Session 06) — a `PROCESSING` payout whose IPN never arrives stays `PROCESSING` forever; there is no sweeper that re-queries Paxum for stale batches. **Still open after Session 11.** Add one when real volume exists (Session 12 candidate).
- ~~**`config.rateLimit` + `keyGenerator: request.user?.userId ?? request.ip` keys on IP, not the user**~~ — **resolved in Session 11.5.** Found in Session 11: `config.rateLimit` runs at `onRequest`, before `authenticate` sets `request.user`. The five affected limits (`PATCH /me/locale`, checkout ×2, messaging send/read, `POST /generations`, cancel/resume) are now `app.rateLimit()` preHandlers placed after auth, and each has a test showing the budget stays with the account when the IP changes. `PUT /payout-email` was listed here, but it never had a per-user `keyGenerator` and is IP-scoped by design.
...
## Last Updated — Session 11.5 complete: rate-limit key hotfix. The five per-user rate limits that were silently keying on IP (`/me/locale`, checkout, messaging send/read, generation create, subscription cancel/resume) are now `app.rateLimit()` preHandlers placed after `authenticate`. No budget value changed. 7 new three-part tests (own-account 429 / same account from a new IP still 429 / another account from the same IP allowed), and a mutation check shows they fail on the old wiring. 381 API tests (+7), zero regressions; `pnpm turbo run typecheck lint test build` and root `pnpm lint` green. **Next: Session 12 (Security Hardening & Performance Audit).** [2026-09-24]

_Previous — Session 11 complete: Admin Dashboard._ New `modules/admin/` behind plugin-scoped `authenticate` + `authorize('admin')` with per-admin rate limits; model approval (`ModelProfile.approvalStatus`, gating upload + subscription checkout with 403 `model_not_approved`); user management with `User.suspendedAt` honoured by login and refresh (403 `account_suspended`) and an admin-cannot-suspend-admin rule; a 7-query per-currency metrics overview (query-count tested); the payout run refactored to one `runPayouts(trigger)` behind both the cron secret and the admin JWT (parity tested); content reports (`Report` + partial unique index, 10/hour per user) with admin resolution through the existing publish toggle and suspend path; and the first admin-facing web pages (`/admin/*`) with a client-side gate and a single `ConfirmAction` component for every state change. Migration `20260920180000_add_admin_dashboard` generated and applied (13 live — Session 10's was found unapplied and went in alongside). 374 API tests (+25) + 39 web tests (+6) + 6 shared, zero regressions; `pnpm turbo run typecheck lint test build` and root `pnpm lint` (jsx-a11y) all green. The four "candidate: Session 11" Open Items (FX, credit-spend sharing, reconciliation sweeper, stale PENDING jobs) were deliberately left open for Session 12. **Next: Session 12 (Security Hardening & Performance Audit).** [2026-09-20]

_Previous — Session 10 complete: i18n & multilingual._ `next-intl` wired end-to-end on the web app with per-request server-side locale resolution (`NEXT_LOCALE` cookie → `Accept-Language` → `NEXT_PUBLIC_DEFAULT_LOCALE`, no path prefix — proven against a real `next build`/`next start` server, not just a mock) and full PT-BR/EN message catalogs covering every existing UI surface (layout, home, wallet, `ProtectedMedia`, the protected-media demo). `User.preferredLocale` (migration `20260920120000_add_user_preferred_locale`, **generated and applied** — 12 migrations live) drives locale-aware transactional email: both the verification and renewal-reminder templates now exist in full in both languages (`Record<Locale, template>` in `email.ts`, no new runtime dependency on the API), with `Intl.NumberFormat`/`Intl.DateTimeFormat` for amounts and dates and `escapeHtml` still guarding every interpolated value in both locales. `SUBSCRIPTION_PLANS`/`CREDIT_PACKS`/`GENERATION_PRESETS` labels are now `LocalizedLabel` records with key parity enforced by the type; `GenerationJob.userPrompt` and payment-provider charge descriptions deliberately keep the canonical English label (see Architecture Decisions). 349 API tests (+18) + 33 web tests (+26, incl. 5 real-server SSR tests) + 6 new shared tests, zero regressions; `pnpm turbo run typecheck lint test build` and root `pnpm lint` (incl. jsx-a11y) all green. **Next: Session 11 (Admin Dashboard).** [2026-09-20]
